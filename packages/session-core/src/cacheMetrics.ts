/**
 * Cache-metrics aggregation over CLI stream events.
 *
 ...
 *
 * Both are validated by the StreamParser in @context-action/code-api. This
 * module layers a pure reducer on top so a running total can be kept on
 * ExecutionInfo while the process streams, and a final snapshot materialized
 * when it completes.
 *
 * Design points:
