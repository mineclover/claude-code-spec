...
#### 2. 배포용 빌드 (권장)
설치 가능한 ZIP 파일을 생성합니다:
```bash
npm run make
```

**결과물:**
- 위치: `out/make/zip/darwin/`
- 파일명: `claude-code-spec-darwin-arm64-1.0.0.zip` (Apple Silicon) 또는 `claude-code-spec-darwin-x64-1.0.0.zip` (Intel)
...
현재는 ZIP 파일만 생성됩니다. DMG 파일을 생성하려면:

1. **forge.config.ts 수정**
   ```typescript
...
| `npm run package` | 빠른 패키징 | `out/claude-code-spec-darwin-*/` |
| `npm run make` | 배포용 빌드 | `out/make/zip/darwin/` |
