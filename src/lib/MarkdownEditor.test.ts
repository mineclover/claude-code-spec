...
`;
  ...
  describe('Region Type Inference', () => {
    it('should infer "section" type', () => {
      const sectionContent = `<!-- MEMORY_START: refs -->
## References
...
<!-- MEMORY_END: refs -->`;
      ...
      const sectionEditor = new MarkdownEditor(sectionContent);
      const items = sectionEditor.parseRegionItems('refs');
      const type = sectionEditor.inferRegionType(items);

      expect(type).toBe('section');
    });

    it('should infer "code" type', () => {
      const codeContent = `<!-- MEMORY_START: tools -->
\`\`\`bash
npm run dev
\`\`\`

\`@context/troubleshooting.md\` - Debug guide
<!-- MEMORY_END: tools -->`;

      const codeEditor = new MarkdownEditor(codeContent);
      const items = codeEditor.parseRegionItems('tools');
      const type = codeEditor.inferRegionType(items);

      expect(type).toBe('code');
    });

    it('should infer "mixed" type', () => {
      const mixedContent = `<!-- MEMORY_START: setup -->
## Setup
...
<!-- MEMORY_END: setup -->`;
      ...
      const mixedEditor = new MarkdownEditor(mixedContent);
      const items = mixedEditor.parseRegionItems('setup');
      const type = mixedEditor.inferRegionType(items);

      expect(type).toBe('mixed');
    });

    it('should auto-update region type after content change', () => {
      const region = editor.findManagedRegion('test-region');
      expect(region?.type).toBe('mixed'); // initial

      // Change to section-only content
      editor.updateManagedRegionContent('test-region', `## References

@context/file1.md
@context/file2.md`);

      const updated = editor.findManagedRegion('test-region');
