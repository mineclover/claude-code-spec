...
function describeZodSchema(schema: ZodType<any, any, any>, indent = 0): string {
  ...
  if (schema instanceof z.ZodObject) {
    ...
    entries.forEach(([key, fieldSchema], index) => {
      ...
      const desc = describeZodField(key, fieldSchema as ZodType<any>, indent + 1);
 ...
 * Zod 필드를 설명
 */
function describeZodField(key: string, schema: ZodType<any>, indent: number): string {
  const _ind = '  '.repeat(indent);
